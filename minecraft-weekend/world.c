	...
	for (int x = 0; x < RENDER_DISTANCE; x++)
		for (int y = 0; y < RENDER_DISTANCE; y++)
			for (int z = 0; z < RENDER_DISTANCE; z++) {
				int index = chunk_coord_to_index(x, y, z);
...
void world_load_unloaded_chunks(struct World* world, int count) {
	...
	for (int chunk_x = 0; chunk_x < RENDER_DISTANCE; chunk_x++)
		...
		num_loaded++;
		if (num_loaded > count) continue;
		chunk = chunk_new((ivec3s) {
			...
			for (int zb = 0; zb < CHUNK_SIZE; zb++) {
				...
				for (int i = 0; i < CHUNK_SIZE; i++) {
					...
					if (world_y == terrain_height) {
						...
						else chunk->blocks[xb][i][zb] = DIRT;
...
bool get_block_at_world_pos(struct World* world, ivec3s world_pos, BlockId* block, struct Chunk** containing_chunk) {
	...
	return chunk_get_block(chunk, block_pos_in_chunk, block);
}